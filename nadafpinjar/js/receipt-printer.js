 ...
 * Exact 3 Receipt Formats:
 * 1. Direct State (Red #990000): 1 Donor Section (Image 4 & 2)
 * 2. Taluk to State (Green #006600): 2 Sections - Taluk President + Donor (Image 3)
 * 3. District to State (Blue #0033cc): 2 Sections - District President + Donor (Image 5)
 */
function generateDonationPDF(data, paymentId, formTitle, formPrefix, subheaderTitle, themeColor) {
    data = data || {};
    formPrefix = formPrefix || 'S';

    // Auto detect type based on prefix or formTitle/subheaderTitle
    let type = 'direct';
    ...
    if (type === 'taluk') {
        themeColor = '#006600'; // Green as in Image 3
        subheaderTitle = 'ತಾಲೂಕಿನಿಂದ ರಾಜ್ಯಕ್ಕೆ ವರ್ಗಾವಣೆ';
    } else if (type === 'district') {
        themeColor = '#0033cc'; // Blue as in Image 5
        subheaderTitle = 'ಜಿಲ್ಲೆಯಿಂದ ರಾಜ್ಯಕ್ಕೆ ವರ್ಗಾವಣೆ';
    } else {
        themeColor = '#990000'; // Red as in Image 4 & 2
    ...
    // Build specific section rows based on receipt type
    let middleRowsHTML = '';

    if (type === 'direct') {
        // Direct State: 1 Donor Section (Image 4 & 2)
        ...
        // Taluk (Image 3) or District (Image 5): 2 Sections
