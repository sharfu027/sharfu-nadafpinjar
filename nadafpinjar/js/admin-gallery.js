    ...
    function renderAdminGrid() {
        ...
        grid.innerHTML = albums.map(alb => {
            ...
            const coverHTML = coverSrc 
                ? `<img src="${coverSrc}" class="album-cover-img" onerror="this.onerror=null; this.src='images/slider-1.jpg';">`
    ...
    function setupEvents() {
        ...
        if (galleryUrlsInput) {
            galleryUrlsInput.addEventListener('change', function(e) {
                ...
                if (val.trim()) {
                    ...
                    urls.forEach(u => {
                        if (!currentGalleryPhotos.includes(u)) currentGalleryPhotos.push(u);
                    });
                    renderGalleryPhotosPreview();
                    e.target.value = '';
                }
        ...
        if (albumForm) {
            albumForm.addEventListener('submit', function(e) {
                ...
                // Ensure any pending text in galleryUrlsInput is captured
                const rawUrlsInput = document.getElementById('galleryUrlsInput');
                if (rawUrlsInput && rawUrlsInput.value.trim()) {
                    const extraUrls = rawUrlsInput.value.split(/[\n,]+/).map(u => u.trim()).filter(Boolean);
                    extraUrls.forEach(u => {
                        if (!currentGalleryPhotos.includes(u)) {
                            currentGalleryPhotos.push(u);
                        }
                    });
                    rawUrlsInput.value = '';
                }
