...
]; // Census.html does not have Razorpay but maybe has its own logic.

for (let f of files) {
    if (!fs.existsSync(f.file)) continue;
    let content = fs.readFileSync(f.file, 'utf8');
    
    // Check if it has razorpay script block
    let rzpIdx = content.indexOf('// Razorpay Configuration');
    if (rzpIdx !== -1) {
        let scriptStart = content.lastIndexOf('<script>', rzpIdx);
            ...
            for (let key in formData) {
                if (key !== 'amount') {
                    ...
                    if (input) {
                        ...
                        if(!label) {
                            // look for closest parent label or previous element
                    ...
                    previewHTML += \`
                        <div class="preview-row">
                            ...
                            <div class="preview-value">\${formData[key] || 'N/A'}</div>
                        </div>
                    \`;
                }
            }
